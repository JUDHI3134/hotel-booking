...
const AddRoom = () => {
  ...
  return (
    <form>
          <Title align='left' font='outfit' title='Add Room' subTitle='Fill in the details carefully and acurate room details, pricing and amenities, to enhance the user booking experiene.' />
          ...
          <div className='grid grid-cols-2 sm:flex gap-4 my-2 flex-wrap'>
              {Object.keys(images).map((key) => (
                  <label htmlFor={`roomImage${key}`} key={key}>
                      <img className='max-h-13 opacity-80 cursor-pointer' src={images[key] ? URL.createObjectURL(images[key]) : assets.uploadArea} alt="" />
          ...
          <div className='w-full flex max-sm:flex-col sm:gap-4 mt-4'>
              <div className='flex-1 max-w-48'>
                  ...
                  <select onChange={e => setInputs({...inputs, roomType: e.target.value})} value={inputs.roomType} className='border border-gray-300 opacity-70 mt-1 rounded w-full p-2'>
