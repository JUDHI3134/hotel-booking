...
const HotelReg = () => {
  ...
  return (
    <div onClick={() => setShowHotelReg(false)} className='fixed top-0 right-0 left-0 bottom-0 z-100 flex items-center justify-center bg-black/70'>
        <form onSubmit={onSubmitHandler} onClick={(e) => e.stopPropagation()} className='flex bg-white rounded-xl max-w-4xl max-md:mx-2'>
              <img src={assets.regImage} alt="reg-image" className='w-1/2 rounded-xl hidd md:block' />   
      ...
      {/* --------Contact Phone----- */}
                  <div className='w-full mt-4'>
                      ...
                      <input type="phone"  onChange={(e) => setContact(e.target.value)} value={contact} id='contact' placeholder='Type here' className='border border-gray-200 rounded w-full px-3 py-2.5 mt-1 outline-indigo-500 font-light' required />
                  </div>
      {/* --------Address----- */}
                  <div className='w-full mt-4'>
                      <label htmlFor="address" className='font-medium text-gray-500'>Address</label>
                      <input type="text"  onChange={(e) => setAddress(e.target.value)} value={address} id='address' placeholder='Type here' className='border border-gray-200 rounded w-full px-3 py-2.5 mt-1 outline-indigo-500 font-light' required />
                  </div>
